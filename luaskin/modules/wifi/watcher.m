...
- (void) _ssidChanged:(id __unused)bla {
    [self performSelector:@selector(ssidChanged:) onThread:_creatingThread
                                       withObject:nil
                                       waitUntilDone:YES];
}
...
static int wifi_watcher_new(lua_State* L) {
    ...
    lua_pushvalue(L, 1);
    wifiwatcher->fn = [skin luaRef:[skin refTableFor:USERDATA_TAG]];
...
static int wifi_watcher_gc(lua_State* L) {
    ...
    wifiwatcher->fn = [skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:wifiwatcher->fn];
