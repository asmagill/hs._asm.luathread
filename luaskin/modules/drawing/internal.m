...
- (void)mouseUp:(NSEvent * __unused)theEvent {
    if (self.mouseUpCallbackRef != LUA_NOREF) {
        LuaSkin *skin = [LuaSkin threaded];
        lua_State *_L = skin.L;
        [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:self.mouseUpCallbackRef];
...
- (void)mouseDown:(NSEvent * __unused)theEvent {
    ...
    if (self.mouseDownCallbackRef != LUA_NOREF) {
        LuaSkin *skin = [LuaSkin threaded];
        lua_State *_L = skin.L;
        [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:self.mouseDownCallbackRef];
        if (![skin protectedCallAndTraceback:0 nresults:0]) {
            const char *errorMsg = lua_tostring(_L, -1);
...
static int disableUpdates(__unused lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TBREAK] ;
...
static int drawing_newText(lua_State *L) {
    ...
    if (theWindow) {
        ...
        theWindow.contentView = theView;
        [theView.textField setAttributedStringValue:[[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSAttributedString"]] ;
...
static int drawing_newImage(lua_State *L) {
    ...
    NSRect windowRect = [skin tableToRectAtIndex:1];
    NSImage *theImage = [[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSImage"];
    HSDrawingWindow *theWindow = [[HSDrawingWindow alloc] initWithContentRect:windowRect styleMask:NSBorderlessWindowMask backing:NSBackingStoreBuffered defer:YES];
...
//         drawingView.textField.attributedStringValue = [[NSAttributedString alloc] initWithString:[NSString stringWithUTF8String:luaL_checkstring(L, -1)] attributes:attributes];
        drawingView.textField.attributedStringValue = [[NSAttributedString alloc] initWithString:[[LuaSkin threaded] toNSObjectAtIndex:-1] attributes:attributes];
        lua_pop(L, 1) ;
    } else {
...
static int drawing_setStyledText(lua_State *L) {
    ...
    if ([drawingView isKindOfClass:[HSDrawingViewText class]]) {
        [drawingView.textField setAttributedStringValue:[[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSAttributedString"]] ;
    } else {
...
static int drawing_getStyledText(lua_State *L) {
    ...
    if ([drawingView isKindOfClass:[HSDrawingViewText class]]) {
        [[LuaSkin threaded] pushNSObject:[drawingView.textField.attributedStringValue copy]] ;
    } else {
        return luaL_argerror(L, 1, "not an hs.drawing text object");
    }
    ...
    if (lua_istable(L, idx)) {
        if (lua_getfield(L, -1, "font")) {
            if (lua_type(L, -1) == LUA_TTABLE) {
                theFont = [[LuaSkin threaded] luaObjectAtIndex:-1 toClass:"NSFont"] ;
            } else {
        ...
        if (lua_getfield(L, -1, "color")) {
            theColor = [[LuaSkin threaded] luaObjectAtIndex:-1 toClass:"NSColor"] ;
        }
        lua_pop(L, 1);

        if (lua_getfield(L, -1, "paragraphStyle")) {
            theStyle = [[[LuaSkin threaded] luaObjectAtIndex:-1 toClass:"NSParagraphStyle"] mutableCopy] ;
            lua_pop(L, 1) ;
        } else {
            lua_pop(L, 1) ;
...
//            [[LuaSkin threaded] logWarn:@"-- unable to retrieve current style for text; reverting to defaults"] ;
        }
...
static int drawing_setTextColor(lua_State *L) {
    drawing_t *drawingObject = get_item_arg(L, 1);
    NSColor *textColor = [[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSColor"] ;
...
static int drawing_setFillColor(lua_State *L) {
    LuaSkin *skin = [LuaSkin threaded];
    drawing_t *drawingObject = get_item_arg(L, 1);
    NSColor *fillColor = [[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSColor"] ;
...
static int drawing_setImage(lua_State *L) {
    LuaSkin *skin = [LuaSkin threaded];
    drawing_t *drawingObject = get_item_arg(L, 1);
    NSImage *image = [[LuaSkin threaded] luaObjectAtIndex:2 toClass:"NSImage"];
...
static int drawing_scaleImage(lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TUSERDATA, USERDATA_TAG,
    ...
    if ([drawingView isKindOfClass:[HSDrawingViewImage class]]) {
        if (lua_type(L, 2) != LUA_TNONE) {
            NSString *arg = [[LuaSkin threaded] toNSObjectAtIndex:2] ;
...
static int drawing_setClickCallback(lua_State *L) {
    ...
    if (lua_type(L, 2) == LUA_TNIL || lua_type(L, 2) == LUA_TFUNCTION) {
        ...
        [drawingView setMouseUpCallback:[skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:drawingView.mouseUpCallbackRef]];

        // Set a new callback if we have a function
        if (lua_type(L, 2) == LUA_TFUNCTION) {
            lua_pushvalue(L, 2);
            [drawingView setMouseUpCallback:[skin luaRef:[skin refTableFor:USERDATA_TAG]]];
        }
    } else {
    ...
    if (lua_type(L, 3) == LUA_TNONE || lua_type(L, 3) == LUA_TNIL || lua_type(L, 3) == LUA_TFUNCTION) {
        // We're either removing a callback, or setting a new one. Either way, we want to make clear out any callback that exists
        [drawingView setMouseDownCallback:[skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:drawingView.mouseDownCallbackRef]];

        // Set a new callback if we have a function
        if (lua_type(L, 3) == LUA_TFUNCTION) {
            lua_pushvalue(L, 3);
            [drawingView setMouseDownCallback:[skin luaRef:[skin refTableFor:USERDATA_TAG]]];
        }
    } else {
...
// NOTE: Change this if you change the defaults in [HSDrawingViewText initWithFrame:]
      [[LuaSkin threaded] pushNSObject:[NSFont systemFontOfSize: 27]] ;                    lua_setfield(L, -2, "font") ;
      [[LuaSkin threaded] pushNSObject:[NSColor colorWithCalibratedWhite:1.0 alpha:1.0]] ; lua_setfield(L, -2, "color") ;
      [[LuaSkin threaded] pushNSObject:[NSParagraphStyle defaultParagraphStyle]] ;         lua_setfield(L, -2, "paragraphStyle") ;
    return 1 ;
}
...
static int drawing_getTextDrawingSize(lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TANY, LS_TTABLE | LS_TNIL | LS_TOPTIONAL, LS_TBREAK] ;
    ...
    switch(lua_type(L, 1)) {
        ...
        case LUA_TUSERDATA:
        case LUA_TTABLE:  {
                NSAttributedString *theText = [[LuaSkin threaded] luaObjectAtIndex:1 toClass:"NSAttributedString"] ;
...
static int drawing_wantsLayer(lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TUSERDATA, "hs.drawing",
                                LS_TBOOLEAN | LS_TOPTIONAL,
                                LS_TBREAK] ;
