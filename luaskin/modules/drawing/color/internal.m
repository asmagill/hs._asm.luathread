...
static int getColorLists(lua_State *L) {
    ...
    for (NSColorList *colorList in [NSColorList availableColorLists]) {
        [[LuaSkin threaded] pushNSObject:colorList] ;
...
static int colorAsRGB(lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TTABLE, LS_TBREAK] ;
    NSColor *theColor = [[LuaSkin threaded] luaObjectAtIndex:1 toClass:"NSColor"] ;
...
// [[LuaSkin threaded] pushNSObject:NSColor]
// C-API
...
static int NSColor_tolua(lua_State *L, id obj) {
    ...
    if (safeColor) {
        ...
        lua_newtable(L) ;
          [[LuaSkin threaded] pushNSObject:[theColor catalogNameComponent]] ;
          lua_setfield(L, -2, "list") ;
          [[LuaSkin threaded] pushNSObject:[theColor colorNameComponent]] ;
          lua_setfield(L, -2, "name") ;
          lua_pushstring(L, "NSColor") ; lua_setfield(L, -2, "__luaSkinType") ;
    } else if ([theColor.colorSpaceName isEqualToString:NSPatternColorSpace]) {
        lua_newtable(L) ;
          [[LuaSkin threaded] pushNSObject:[theColor patternImage]] ;
...
// [[LuaSkin threaded] pushNSObject:NSColorList]
// C-API
...
static int NSColorList_tolua(lua_State *L, id obj) {
    ...
    for (id key in [colorList allKeys]) {
        [[LuaSkin threaded] pushNSObject:[colorList colorWithKey:key]] ;
    ...
    if (level < COLOR_LOOP_LEVEL) {
        ...
        if (colorList && colorName && !image) {
            ...
            if (colorCollectionsTable != LUA_NOREF) {
                [skin pushLuaRef:[skin refTableFor:"hs.drawing.color"] ref:colorCollectionsTable] ;
...
// [[LuaSkin threaded] luaObjectAtIndex:idx toClass:"NSColor"]
// C-API
...
static int registerColorCollectionsTable(lua_State *L) {
    ...
    lua_pushvalue(L, 1) ;
    colorCollectionsTable = [[LuaSkin threaded] luaRef:[[LuaSkin threaded] refTableFor:"hs.drawing.color"]] ;
    return 0 ;
}
