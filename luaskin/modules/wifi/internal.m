...
static int __unused log_to_console(lua_State *L, const char *level, NSString *theMessage) {
    ...
    if (logFnRef != LUA_NOREF) {
        [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:logFnRef];
    ...
    if (![[LuaSkin threaded] protectedCallAndTraceback:1 nresults:0]) { return lua_error(L); }
    return 0;
}

static int lua_registerLogForC(__unused lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TTABLE, LS_TBREAK];
    logFnRef = [[LuaSkin threaded] luaRef:[[LuaSkin threaded] refTableFor:USERDATA_TAG]];
    return 0;
}
...
@property NSThread *creatingThread ;
@end
...
- (id)initWithCallback:(int)fnReference onInterface:(NSString *)interface {
    ...
    if (self) {
        ...
        _creatingThread = [NSThread currentThread] ;
    }
    return self ;
}
...
- (void)doBackgroundScan:(id)object {
    ...
    _isDone = YES ;
    if (theError) {
        [self performSelector:@selector(invokeCallback:) onThread:_creatingThread
                           withObject:theError
                        waitUntilDone:NO];
    } else {
        [self performSelector:@selector(invokeCallback:) onThread:_creatingThread
...
- (void)invokeCallback:(id)object {
    if (_fnRef != LUA_NOREF) {
        LuaSkin *skin = [LuaSkin threaded] ;
        lua_State *L = [skin L] ;
        [[LuaSkin threaded] pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:_fnRef];
...
static int wifi_scan_background(lua_State* L __unused) {
    ...
    if (lua_type(L, 1) != LUA_TNIL) {
        lua_pushvalue(L, 1);
        callbackRef = [skin luaRef:[skin refTableFor:USERDATA_TAG]];
    }
...
static int userdata_gc(lua_State* L) {
    ...
    scanner.fnRef = [skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:scanner.fnRef];
