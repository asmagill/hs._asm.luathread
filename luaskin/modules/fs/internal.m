...
const char *path_at_index(lua_State *L, int i) {
    NSString *path = [[LuaSkin threaded] toNSObjectAtIndex:i];
...
NSArray *tags_from_lua_stack(lua_State *L) {
    ...
    while (lua_next(L, 2) != 0) {
        if (lua_type(L, -1) == LUA_TSTRING) {
            NSString *tag = [[LuaSkin threaded] toNSObjectAtIndex:-1];
            [tags addObject:tag];
        }
        lua_pop(L, 1);
    }
...
#pragma clang diagnostic pop
        [[LuaSkin threaded] logError:[NSString stringWithFormat:@"hs.fs tags_from_file() Unable to get tags for %@: %@", url, [error localizedDescription]]];
        return nil;
    }
    return tags;
}
...
#pragma clang diagnostic pop
        [[LuaSkin threaded] logError:[NSString stringWithFormat:@"hs.fs tags_to_file() Unable to set tags for %@: %@", url, [error localizedDescription]]];
        return false;
    }
    return true;
}
...
static int change_dir (lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TSTRING, LS_TBREAK];
...
static int make_link(lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TSTRING, LS_TSTRING, LS_TBOOLEAN|LS_TOPTIONAL, LS_TBREAK];
...
static int file_utime (lua_State *L) {
    [[LuaSkin threaded] checkArgs:LS_TSTRING, LS_TNUMBER|LS_TOPTIONAL, LS_TNUMBER|LS_TOPTIONAL, LS_TBREAK];
...
static int _file_info_ (lua_State *L, int (*st)(const char*, STAT_STRUCT*)) {
    [[LuaSkin threaded] checkArgs:LS_TSTRING, LS_TSTRING|LS_TOPTIONAL, LS_TBREAK];
    const char *file = path_at_index(L, 1);
    STAT_STRUCT info;
    int i;
