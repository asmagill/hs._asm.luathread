...
- (void)callback:(NSTimer *)timer {
    ...
    [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:self.fnRef];
...
static int timer_new(lua_State* L) {
    ...
    lua_pushvalue(L, 2);
    int callbackRef = [skin luaRef:[skin refTableFor:USERDATA_TAG]];
...
static int timer_gc(lua_State* L) {
    ...
    if (timer) {
        [timer stop];
        timer.fnRef = [skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:timer.fnRef];
        timer.t = nil;
        timer = nil;
    }
