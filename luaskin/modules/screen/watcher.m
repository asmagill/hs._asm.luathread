...
- (void) _screensChanged:(id)note {
    [self performSelector:@selector(screensChanged:) onThread:self.creatingThread
                                        withObject:note
                                        waitUntilDone:YES];
}
...
- (void) screensChanged:(NSNotification*)note {
    ...
    [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:self.fn];
    if (_includeActive) {
...
static int screen_watcher_new(lua_State* L) {
    ...
    lua_pushvalue(L, 1);
    screenwatcher->fn = [skin luaRef:[skin refTableFor:USERDATA_TAG]];

    MJScreenWatcher* object = [[MJScreenWatcher alloc] init];
    object.L = L;
    object.fn = screenwatcher->fn;
    object.includeActive = NO;
    object.creatingThread = [NSThread currentThread] ;
...
static int screen_watcher_gc(lua_State* L) {
    ...
    screenwatcher->fn = [skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:screenwatcher->fn];
