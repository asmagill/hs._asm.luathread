...
@property int ref;
@property NSThread *callingThread ;
@end

@implementation MJKeycodesObserver

- (void) _inputSourceChanged:(NSNotification*)__unused note {
    [self performSelector:@selector(inputSourceChanged:) onThread:self.callingThread
                                       withObject:nil
                                    waitUntilDone:YES];
}
...
- (void) inputSourceChanged:(NSNotification*)__unused note {
    ...
    lua_State *L = skin.L;
    [skin pushLuaRef:[skin refTableFor:USERDATA_TAG] ref:self.ref];
...
static int keycodes_newcallback(lua_State* L) {
    ...
    lua_pushvalue(L, 1);
    int ref = [skin luaRef:[skin refTableFor:USERDATA_TAG]];

    MJKeycodesObserver* observer = [[MJKeycodesObserver alloc] init];
    observer.ref = ref;
    observer.callingThread = [NSThread currentThread] ;
    [observer start];
...
static int keycodes_callback_gc(lua_State* L) {
    ...
    observer.ref = [skin luaUnref:[skin refTableFor:USERDATA_TAG] ref:observer.ref];
    observer = nil;
    return 0;
}
